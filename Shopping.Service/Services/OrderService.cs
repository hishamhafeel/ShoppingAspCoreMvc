    ...
    public class OrderService : IOrderService
    {
        ...
        public void AddOrder(OrderBO order)
        {
            ...
            using (var transaction = unitOfWork.Context.Database.BeginTransaction())
            {
                try
                {
                    var model = new Order(order.OrderDate, order.OrderTotal, order.CustomerId, mapper.Map<List<OrderItem>>(order.OrderItems));
                    var result = model.Create(model);
                    unitOfWork.OrderRepository.Create(result);
                    unitOfWork.Save();
